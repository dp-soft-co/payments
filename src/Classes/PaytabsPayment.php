...
class PaytabsPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->paytabs_profile_id = config('dpsoft-payments.PAYTABS_PROFILE_ID');
        $this->paytabs_base_url = config('dpsoft-payments.PAYTABS_BASE_URL');
        $this->paytabs_server_key = config('dpsoft-payments.PAYTABS_SERVER_KEY');
        $this->paytabs_checkout_lang = config('dpsoft-payments.PAYTABS_CHECKOUT_LANG');
        $this->currency = config('dpsoft-payments.PAYTABS_CURRENCY');
    ...
    public function pay(
        ...
        $response = Http::withHeaders([
            ...
            "callback" => route($this->verify_route_name, ['payment_id' => $unique_id, 'gateway' => "paytabs"]), //Post end point  -the payment status will be sent to server
            "return" => route($this->verify_route_name, ['payment_id' => $unique_id, 'gateway' => "paytabs"]), //Get end point - The link to which the user will be redirected
