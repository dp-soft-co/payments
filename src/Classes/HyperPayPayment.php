...
class HyperPayPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->hyperpay_url = config('dpsoft-payments.HYPERPAY_URL');
        $this->hyperpay_base_url = config('dpsoft-payments.HYPERPAY_BASE_URL');
        $this->hyperpay_token = config('dpsoft-payments.HYPERPAY_TOKEN');
        $this->currency = config('dpsoft-payments.HYPERPAY_CURRENCY');
        $this->hyperpay_credit_id = config('dpsoft-payments.HYPERPAY_CREDIT_ID');
        $this->hyperpay_mada_id = config('dpsoft-payments.HYPERPAY_MADA_ID');
        $this->hyperpay_apple_id = config('dpsoft-payments.HYPERPAY_APPLE_ID');
    ...
    public function verify(Request $request)
    {
        ...
        if (in_array($final_result["result"]["code"], ["000.000.000", "000.100.110", "000.100.111", "000.100.112"])) {
            ...
            return [
                'success' => false,
                'payment_id'=>$request['id'],
                'message' => __('dpsoft::messages.PAYMENT_FAILED_WITH_CODE', ['CODE' => $final_result["result"]["code"]]),
                'process_data' => $final_result
            ];
        }
    }
    ...
    {
        return view('dpsoft::html.hyper_pay', ['model' => $this, 'brand' => $this->getBrand()])->render();
    }
